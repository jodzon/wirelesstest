...
test('ScrapCommand class validateUrl test', function () use ($properURL, $badURL) {
    $this->scrapMock =  new ScrapCommand();
    $checkUrl = env('VIDEX_SCRAP_URL');
    expect($this->scrapMock->validateUrl($properURL))->toBeTrue();
    expect($this->scrapMock->validateUrl($badURL))->toBeNull();
});

test('ScrapCommand class sortResults test', function () {
    $this->scrapMock =  new ScrapCommand();
    $inputArray = [
        ['option_price' => 1],
        ['option_price' => 10],
